...
$(function () {
  ...
  var errorHandler = function (err) {
    console.log(err);
    showToaster({message: 'Looks like something fucked up! Check the console...', type: 'error'});
  }
  ...
  $('#show-linked-artists').click(() => {
    $.ajax('/api/data/user/showlinkedartists', {
      method: 'post',
      data: {
      ...
      success: (result) => {
        ...
        $.each(result, (index, item) => {
          ...
          if (!links[aId]) {
            links[aId] = {artist: item.artist.properties.name, gigs: []};
          }
        ...
        $.each(links, (index, item) => {
          $('#linked-artists-results').append($('<div>').text([item.artist, 'at', item.gigs.join(' & ')].join(' ')));
        })
      },
      error: errorHandler
    });
    ...
    $.ajax('/api/data/user/feed', {
      ...
      success: (result) => {
        ...
        $.each(feedItems, (index, item) => {
          $('#user-feed-results').append($('<div>').text([item.artist, 'at', item.gigs.join(' & ')].join(' ')));
        })
      },
      error: errorHandler
    })
  ...
  let renderUsers = (personList) => {
    ...
    $.each(personList, (i, item) => {
      ...
      var userElement = $('<div>');
      userElement.html([person.emailId, person.name, person.age].join('; '));
      userListContainer.append(userElement);

      var userOption = $('<option>').attr({value: item.person._id}).text(person.emailId);
  ...
  let renderHosts = (hostList) => {
    ...
    $.each(hostList, (i, item) => {
      ...
      var hostOption = $('<option>').attr({value: item.host._id}).text(host.name);
  ...
  let renderGigs = (gigsList) => {
    ...
    $.each(gigsList, (i, item) => {
      ...
      var option = $('<option>').attr({value: item.gig._id}).text(gig.name);
  ...
  let renderArtists = (artistList) => {
    ...
    $.each(artistList, (i, item) => {
      ...
      var userElement = $('<div>');
      userElement.html([artist.name, artist.age, artist.genre].join('; '));
      listContainer.append(userElement);

      var userOption = $('<option>').attr({value: item.artist._id}).text(artist.name);
